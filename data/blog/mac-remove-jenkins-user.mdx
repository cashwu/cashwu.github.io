...
接續上一篇 [MAC 安裝 Jenkins](/blog/mac-install-jenkins)，在 MAC 安裝 Jenkins 時，會順便建立一個使用者來運行 Jenkins，如果只是在自己的本機運行，沒有安全性的疑慮，是可以把這個使用者移除的，不然每次登入就要先選擇使用者，其實有點麻煩，下面就來看如何移除這個使用者吧 !

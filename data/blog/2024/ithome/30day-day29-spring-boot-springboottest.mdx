...
![](/static/images/2024/ithome/30day-day29-spring-boot-springboottest/cover.webp)
...
- `@SpringBootTest` 啟動完整的應用程式
  - 使用 `webEnvironment = WebEnvironment.RANDOM_PORT` 時，Spring Boot 會啟動一個測試用的應用程式，並使用一個`隨機`的可用 `port 號`

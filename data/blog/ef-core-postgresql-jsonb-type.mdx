...
前面的[文章](/blog/postgresql-json-jsonb)有大概介紹了 PostgreSQL Json 和 Jsonb 的不同，現在要來看如何在 EF Core 裡面使用 Jsonb
...
- 不過拿到字串的話，在程式還需要在轉換一次才方便使用，所以我們可以使用前面[文章](/blog/ef-core-value-converter)有提過的 `Value Converter` 來 mapping 成強型別

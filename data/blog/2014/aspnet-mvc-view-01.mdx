...
![](/static/images/2014/aspnet-mvc-view-01/01.webp)

- `CodeInline` 用在一行程式碼 (一行的陳述式)

![](/static/images/2014/aspnet-mvc-view-01/02.webp)

- `CodeBlock` 用在多行程式碼，裡面要加分號結尾

![](/static/images/2014/aspnet-mvc-view-01/03.webp)

- 在 `CodeBlock` 裡面輸出 `Html`，在每一行開頭加 `@：`

![](/static/images/2014/aspnet-mvc-view-01/04.webp)

- 除了在開頭加 `@：` 之外還可以用 `<tag>`，而且不管 `<tag>` 有沒有意義，被包起來就會輸出成 `Html` 而且 `tag` 會被輸出

![](/static/images/2014/aspnet-mvc-view-01/05.webp)

- `<text>` 是 `Razor` 專屬的語法，`tag` 並不會被輸出，所以可以用來精準的控制 Html

![](/static/images/2014/aspnet-mvc-view-01/06.webp)

- 如果要輸出 `@`，需要使用 `@` 當跳脫字元

![](/static/images/2014/aspnet-mvc-view-01/07.webp)

- 為了避免 `xss` 攻擊，預設所有輸出的內容都會 `HtmlEncode`，如果要原封不動的輸出可以用 `@Html.Raw( )`

![](/static/images/2014/aspnet-mvc-view-01/08.webp)

## Layout 主版頁面

在 `MVC` 的專案範本中有一個預設的 `/View/_ViewStart.cshtml` 檔案，個檔案會在任何 `View` 被載入前先被載入

![](/static/images/2014/aspnet-mvc-view-01/09.webp)

而檔案的內容只有一行，定義了 `layout` (主版頁面) 的檔案路徑

![](/static/images/2014/aspnet-mvc-view-01/10.webp)
...
![](/static/images/2014/aspnet-mvc-view-01/11.webp)
...
![](/static/images/2014/aspnet-mvc-view-01/12.webp)

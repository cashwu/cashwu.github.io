...
![](/static/images/2014/aspnet-mvc-db-dev-01/01.webp)

在資料庫上點兩下應該會用 `伺服器管理員` 開啟，確認可以連接到資料庫

![](/static/images/2014/aspnet-mvc-db-dev-01/02.webp)

在 `Model` 資料夾上按右鍵加入一個 `資料` > `ADO.NET Entity Data Model` > 名稱打 `Customer`

![](/static/images/2014/aspnet-mvc-db-dev-01/03.webp)

選第一個 從資料庫產生模型

![](/static/images/2014/aspnet-mvc-db-dev-01/04.webp)

預設會選取剛才有打開過的資料庫，如果這裡空的話就要按右邊的 `新連線` 手動加入資料庫，連線字串預設即可

![](/static/images/2014/aspnet-mvc-db-dev-01/05.webp)

加入所有的資料表，`sysdiagrams` 為資料庫圖表可以不加入，命名空間預設即可

![](/static/images/2014/aspnet-mvc-db-dev-01/06.webp)

完成之後就會產生資料庫圖表，並且在 `Model` 裡面產生幾個檔案

![](/static/images/2014/aspnet-mvc-db-dev-01/07.webp)

![](/static/images/2014/aspnet-mvc-db-dev-01/08.webp)
...
![](/static/images/2014/aspnet-mvc-db-dev-01/09.webp)
...
![](/static/images/2014/aspnet-mvc-db-dev-01/10.webp)

打開之後可以看到主要分為三個部份

第一個部份是 SSDL (StorageModels) 這個部份放的是資料庫的資料

![](/static/images/2014/aspnet-mvc-db-dev-01/11.webp)

第二個部份是 CSDL (ConceptualModels) 這個部份放的是程式碼的部份

![](/static/images/2014/aspnet-mvc-db-dev-01/12.webp)

第三個部份是 MSL(C-S mapping) 這個部份放的是資料庫和程式碼的對應關系

![](/static/images/2014/aspnet-mvc-db-dev-01/13.webp)
...
![](/static/images/2014/aspnet-mvc-db-dev-01/14.webp)
...
![](/static/images/2014/aspnet-mvc-db-dev-01/15.webp)

可以在 `專案資料夾` > `bin 資料夾` > `Models 資料夾`，裡面看到那三個檔案，
主要是用副檔名來作區分

![](/static/images/2014/aspnet-mvc-db-dev-01/16.webp)

如果用 `VisualStudio` 開啟，看到的內容就會跟在 `EF` 裡面看到的一樣

![](/static/images/2014/aspnet-mvc-db-dev-01/17.webp)

如果打開看連線字串的話，會發現裡面 `metadata` 裡面的位置改成了 `bin 資料夾` 的位置

![](/static/images/2014/aspnet-mvc-db-dev-01/18.webp)

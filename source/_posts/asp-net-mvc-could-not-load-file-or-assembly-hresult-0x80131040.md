...
![](./asp-net-mvc-could-not-load-file-or-assembly-hresult-0x80131040/01.png)

今天在開發 Web 專案時，突然發生 `無法載入檔案或組件 'Newtonsoft.Json' 或其相依性的其中之一。 找到的組件資訊清單定義與組件參考不符。 (發生例外狀況於 HRESULT: 0x80131040)`

## 原因

在加入專案的某套件時，被自動加入了 `Newtonsoft.Json` 這個套件，而在 references 的專案中也有用到 Newtonsoft.Json 這個套件，不過使用的版本和 Web 的不一樣，導致套件的版本錯亂

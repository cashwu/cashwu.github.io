...
![](./opserver-monitoring-sql-server/07.png)

目前的設定為 `alladmin`, 所以不用打帳號和密碼直接 Login 就好了，不過因為還沒有設定相關監控的 Config 所以會發生沒有 Configuration 的錯誤

![](./opserver-monitoring-sql-server/08.png)

4、About 頁面

![](./opserver-monitoring-sql-server/09.png)

如果連到 about 頁面的話，可以看到整個網站的情況

5、設置 SQLSettings.json

![](./opserver-monitoring-sql-server/10.png)
...
![](./opserver-monitoring-sql-server/11.png)
...
![](./opserver-monitoring-sql-server/12.png)
...
![](./opserver-monitoring-sql-server/13.png)

可以看到 SQL Server 的相關資訊，下面就簡單的介紹一下可點連結的內容

點選 `Databases`，和點選 nav bar 的 top 一樣

![](./opserver-monitoring-sql-server/14.png)

點選 `Jobs`，會跳出一個視窗顯示目前的所有的 SQL Server jobs

![](./opserver-monitoring-sql-server/16.png)

> 注意：這裡可以把 job 停止或是啟動

點選 View Configuration，會跳出一個視窗顯示 SQL Server 相關的屬性資料

![](./opserver-monitoring-sql-server/17.png)

![](./opserver-monitoring-sql-server/18.png)

點選 `CPU`，和點選 nav bar 的 top 一樣

![](./opserver-monitoring-sql-server/19.png)

![](./opserver-monitoring-sql-server/20.png)

這個部份等後面講 databases 的時候在看

點選 `Memory`，會跳出一個視窗顯示目前 SQL Server 使用記憶體的詳細情況

![](./opserver-monitoring-sql-server/21.png)

![](./opserver-monitoring-sql-server/22.png)

點選 `Connections`，會跳出一個視窗顯示 SQL Server 登入情況和做了幾個動作 (Connection)

![](./opserver-monitoring-sql-server/23.png)

點選視窗右上角的 View Detail，和點選 nav bar 的 connections 一樣

點選 `Volumes`，會跳出一個視窗顯示 SQL Server 相關的檔案情況

![](./opserver-monitoring-sql-server/24.png)

![](./opserver-monitoring-sql-server/25.png)

不過點選視窗右上角的 View Detail 的話，不知道為什麼會找不到頁面，還沒有仔細的研究

top

![](./opserver-monitoring-sql-server/26.png)

不知道為什麼裡面沒有資料，之後研究出來再更新

active

![](./opserver-monitoring-sql-server/27.png)

如果執行時發生 `sp_WhoIsActive` 錯誤的話，請參考這個網頁的解法 [sp_whoisactive and Azure SQL Database](http://sqlblog.com/blogs/adam_machanic/archive/2015/04/10/sp-whoisactive-and-azure-sql-database.aspx)，
就會出現頁面了

![](./opserver-monitoring-sql-server/28.png)

其實這個頁面是看 Deadlock 的狀態，如果自己製造一個 Deadlock 就可以看到資料了

![](./opserver-monitoring-sql-server/29.png)

connections

![](./opserver-monitoring-sql-server/30.png)

可以看到所有 `connection` 的情況

![](./opserver-monitoring-sql-server/31.png)

點選一筆的話，會跳出視窗顯示詳細的 Query 內容

![](./opserver-monitoring-sql-server/32.png)

![](./opserver-monitoring-sql-server/33.png)

而且還可以看到執行計畫，滑鼠移上去也會顯示詳細內容，和在 SSMS 裡面看到的差不多喔 !!

![](./opserver-monitoring-sql-server/34.png)

不過有的時候點選會出現問題，目前還沒有研究為什麼會出錯

![](./opserver-monitoring-sql-server/35.png)

databases

![](./opserver-monitoring-sql-server/36.png)

顯示每個 DB 的目前情況

點選其中一筆的話會跳出一個視窗顯示 DB 詳細的內容，不過不知道為什麼我只可以點選 `Tables`，其它的內容就等研究出來再更新

![](./opserver-monitoring-sql-server/37.png)

...
-  C# 在 String 的使用上會被優化，只是因為實現的方式不一樣，所以優化的方式也會不太一樣 
	...
	- String.Empty 在 C# 的內部的程式碼是 public static readonly String Empty (可參考 String source code)，因為是 static 的，所以是 C# 語法級別的優化。當程式碼被編譯為 IL時，才會對程式碼進行優化，即所有對 String 類的靜態字段 Empty 的訪問都會被指向同一引用
    
	- "" 是由 CLR 進行優化的。是通過 String Pool 的方式來避免重新建立相同的字串

- 下圖為跑 10億次的結果，原則上兩個是差不多的，有的時候會一樣，所以效能的差異上可以完全忽略

![](./csharp-string-empty/01.png)
...
- [C# string.Empty和""效率比较](http://www.liangshunet.com/ca/201306/721131003.htm)
- [[.Net Concept]理解並善用String pool](http://www.dotblogs.com.tw/larrynung/archive/2011/06/30/30763.aspx)
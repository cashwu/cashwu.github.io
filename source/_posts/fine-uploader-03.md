...
- 註 2、對 `HttpPostedFileBase` 不熟的可以去看 kevin大 寫的文章，[ASP.NET MVC - 檔案上傳的基本操作](http://kevintsengtw.blogspot.tw/2013/03/aspnet-mvc.html)
...
![](./fine-uploader-03/01.png)

debug 可以看到傳進來的檔案了

![](./fine-uploader-03/02.png)

如果要自定義這個參數的話，也是可以

`JS` 修改如下：

![](./fine-uploader-03/03.png)

在這裡把名稱改為 `uploadfile`，相對的 `controller` 裡面的名稱也要修改

debeg 可以看到，一樣可以接到檔案

![](./fine-uploader-03/04.png)
...
![](./fine-uploader-03/08.png)

可以在 `function` 裡面寫 `console.log`，先測試執行的順序

![](./fine-uploader-03/09.png)

先在 html 裡面增加一個 `input` id為 `params`，

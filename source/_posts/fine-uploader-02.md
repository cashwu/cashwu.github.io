...
![](./fine-uploader-02/02.png)

`js` 如下：

![](./fine-uploader-02/03.png)

`controller` 如下：

![](./fine-uploader-02/04.png)

---

執行程式如果沒有問題的話，畫面上應該會有一個紅色的 `Button`

![](./fine-uploader-02/05.png)

可以 Debug 看看是不是有接到前端上傳的檔案

![](./fine-uploader-02/06.png)

不過如果後端什麼都不作，直接返回一個空的 `json`，這樣子前端會出錯

![](./fine-uploader-02/07.png)

我們可以返回一個 `success` 為 `true`，這樣子前端就會成功了

![](./fine-uploader-02/08.png)

---

程式如下：

![](./fine-uploader-02/09.png)

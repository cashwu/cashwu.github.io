...
![](./sql-storeprocedure-input-date-parameter/01.png)

![](./sql-storeprocedure-input-date-parameter/02.png)

下面為測試的三個 SP，原則上就傳入時的參數型態不同而已 (對應到 Table 裡面的三個時間的型態)

![](./sql-storeprocedure-input-date-parameter/03.png)

![](./sql-storeprocedure-input-date-parameter/04.png)

![](./sql-storeprocedure-input-date-parameter/05.png)

下面為使用 Linqpad 測試的回傳結果

![](./sql-storeprocedure-input-date-parameter/06.png)

![](./sql-storeprocedure-input-date-parameter/07.png)

![](./sql-storeprocedure-input-date-parameter/08.png)

其實很明顯可以看的出來，如果只是使用等於的話，除非傳入參數的型態和資料欄位的型態一樣都是 `Date`，而且程式端需要使用 `DateTime.Today` 傳入，不然的話都查不到資料。所以其實影響查詢的重點有三個：程式端參數、SP參數和資料欄位，不光只有程式端注意就好了，SP也是很大的重點
